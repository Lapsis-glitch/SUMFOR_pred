# collect_training_data_hybrid.py
...
OUT_CSV     = "training_fragments_SIP_hybrid_BDE.csv"
...
def main():
    ...
    with open(OUT_CSV, "w", newline="") as f_out:
        ...
        for entry_id in entry_ids:
            ...
            # --- Patch: ensure BDE fragments always survive physics ---
            orig_compute_conf = engine._compute_confidences

            def patched_compute_confidences(raw_scores):
                conf = orig_compute_conf(raw_scores)
                candidates = getattr(engine, "_current_candidates", None)
                if candidates is None:
                    return conf
                for i, (_, rule_source, _) in enumerate(candidates):
                    if "bde" in rule_source and conf[i] == 0.0:
                        conf[i] = 1e-6
                return conf

            engine._compute_confidences = patched_compute_confidences
            ...
            for a in assignments:
                ...
                # Pathway features
                is_bde_fragment = int("bde" in rule_source.lower())
                n_bde_steps = rule_source.lower().count("bde")
                n_rule_steps = len(rule_source.split("+")) - n_bde_steps
                path_length = len(rule_source.split("+"))

                contains_aromatic_rule = int("aromatic" in rule_source.lower())
                contains_neutral_loss = int("neutral" in rule_source.lower())
                contains_rearrangement = int("rearrang" in rule_source.lower())

                # Fragment structural features
                mass_defect = frag_mass - round(frag_mass)
                H_to_C_ratio = frag_n_H / frag_n_C if frag_n_C > 0 else 0
                N_to_C_ratio = frag_n_N / frag_n_C if frag_n_C > 0 else 0
                O_to_C_ratio = frag_n_O / frag_n_C if frag_n_C > 0 else 0

                # EI canonical ions (very predictive)
                is_common_ei_ion = int(round(frag_mass) in {39, 51, 65, 77, 91, 105})

                # Peak proximity features
                distances = [abs(m - frag_mass) for m in nist_mz]
                distance_to_nearest_peak = min(distances) if distances else 999

                if distances:
                    nearest_idx = distances.index(distance_to_nearest_peak)
                    intensity_of_nearest_peak = nist_int[nearest_idx]
                else:
                    intensity_of_nearest_peak = 0.0

                within_1Da = int(distance_to_nearest_peak <= 1.0)
                within_2Da = int(distance_to_nearest_peak <= 2.0)
                ...
                row = {
                    ...
                    "H_to_C_ratio": H_to_C_ratio,
                    "N_to_C_ratio": N_to_C_ratio,
                    "O_to_C_ratio": O_to_C_ratio,
                    "is_common_ei_ion": is_common_ei_ion,
                    "distance_to_nearest_peak": distance_to_nearest_peak,
                    "intensity_of_nearest_peak": intensity_of_nearest_peak,
                    "within_1Da": within_1Da,
                    "within_2Da": within_2Da,
