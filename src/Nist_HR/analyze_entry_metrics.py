...
Post-hoc analysis of entry-level validation results.

Reads the ``entry_metrics.csv`` (or ``.jsonl``) produced by a validation run,
enriches every entry with chemical descriptors and BDE / spectrum statistics
from the merged JSON dataset, and generates:
  - One scatter- or box-plot per descriptor vs. precision.
  - A correlation heatmap across all numeric descriptors.

Usage
-----
CLI::

    python analyze_entry_metrics.py <path/to/entry_metrics.csv>
    python analyze_entry_metrics.py --test   # quick sanity check on 5 rows

IDE (PyCharm)::

    Run directly — the script will prompt for the input file interactively.
"""
...
# Bond types to count individually
_BOND_TYPES = [
    "C-C", "C-H", "C-O", "C-N",
    "C-Cl", "C-Br", "C-F", "C-I",
    "O-H", "N-H",
...
# Full list of descriptor column names (used for plotting + correlation)
DESCRIPTOR_NAMES = [
    # Atom counts
    ...
    "has_ring", "is_aromatic", "is_heterocycle",
    "is_halogenated", "is_nitrogenous", "is_oxygenated", "is_sulfur",
    # Element ratios
    "H_C", "O_C", "N_C", "S_C", "Hal_C",
    # BDE statistics
    "bde_min", "bde_max", "bde_mean", "bde_median",
    "bde_count", "bde_lt80", "bde_lt100", "bde_lt120",
    ...
    # Retention / match
    "ri", "match_score",
] + [f"bonds_{t}" for t in _BOND_TYPES]


# ── Descriptor extraction from parent formula ───────────────

def extract_descriptors(formula_str):
    """
    Parse a molecular formula string and return a dict of chemical descriptors.

    Descriptors include atom counts, DBE, exact mass, element ratios,
    and formula-based chemical-class flags (aromatic, halogenated, etc.).
    """
    ...
    desc = {
        ...
        "num_P": elems.get("P", 0),
        "num_F": elems.get("F", 0),
        "num_Cl": elems.get("Cl", 0),
        "num_Br": elems.get("Br", 0),
        "num_I": elems.get("I", 0),
    }
    desc["num_halogen"] = desc["num_F"] + desc["num_Cl"] + desc["num_Br"] + desc["num_I"]
    desc["dbe"] = dbe(f)
    desc["exact_mass"] = exact_mass(f, charged=False)

    # Element ratios (None when denominator is zero)
    nc = desc["num_C"] or None
    desc["H_C"] = desc["num_H"] / nc if nc else None
    desc["O_C"] = desc["num_O"] / nc if nc else None
    desc["N_C"] = desc["num_N"] / nc if nc else None
    desc["S_C"] = desc["num_S"] / nc if nc else None
    desc["Hal_C"] = desc["num_halogen"] / nc if nc else None

    # Chemical-class flags derived from formula
...
# ── BDE and spectrum statistics from the merged JSON ─────────

def extract_merged_metrics(entry_id, merged):
    """
    Look up *entry_id* in *merged* and return BDE statistics,
    spectrum peak counts / intensities, retention index, and match score.
    """
    d = merged.get(str(entry_id), {})
    out = {}

    # ── BDE data ─────────────────────────────────────────────
    bde_raw = d.get("bde_data")
    bde = bde_raw if isinstance(bde_raw, dict) else {}
    bonds = bde.get("bonds") or []
    ...
    for t in _BOND_TYPES:
        ...
        out[f"bonds_{t}"] = sum(
            1 for b in bonds
            if b and b.get("bde") is not None
            and (
                (b.get("atom1_symbol") == a1 and b.get("atom2_symbol") == a2)
                or (b.get("atom1_symbol") == a2 and b.get("atom2_symbol") == a1)
            )
        )

    # ── NIST spectrum statistics ─────────────────────────────
...
def main(entry_metrics_path):
    """
    Load entry metrics, enrich with descriptors, generate plots and
    a correlation heatmap.
    """
    # Auto-detect format (CSV or JSONL)
    ...
    # Load the merged JSON for extra metrics (BDE, spectrum stats)
    with open(MERGED_PATH, "r") as f:
        merged = json.load(f)

    # ── Enrich dataframe with computed descriptors ───────────
    desc_df = pd.DataFrame(df["parent_formula"].apply(extract_descriptors).tolist())
    merged_df = pd.DataFrame(
        df["entry_id"].apply(lambda eid: extract_merged_metrics(eid, merged)).tolist()
    )
    df = pd.concat([df, desc_df, merged_df], axis=1)

    # ── Generate per-descriptor plots ────────────────────────
    ...
    corr_cols = ["precision"] + [
        d for d in DESCRIPTOR_NAMES
        if d in df.columns and pd.api.types.is_numeric_dtype(df[d])
    ]
    non_numeric = [
        d for d in DESCRIPTOR_NAMES
        if d in df.columns and not pd.api.types.is_numeric_dtype(df[d])
    ]
    if non_numeric:
        print(f"[WARN] Dropping non-numeric columns from correlation: {non_numeric}")
...
if __name__ == "__main__":
    if len(sys.argv) >= 2 and sys.argv[1] == "--test":
        # Quick sanity check: print descriptors for the first 5 entries
        test_path = os.path.join(
            os.path.dirname(__file__),
            "validation_outputs", "2026-03-26_16-25-23", "entry_metrics.csv",
        )
        ...
        user_path = input().strip() or default_path
